...
from xmop.xmop_s_planning_policy import SinglestepPosePlanningPolicy
...
for idx in random_indices:
    ...
    for _ in range(MAX_ROLLOUT_STEPS):
        ...
        with torch.no_grad():
            rel_poses_p = model.get_action(data_dict_batch)
        rel_poses_p = rel_poses_p[~data_dict_batch['dof_mask']]

        TAB_p = torch.zeros((dof+1,4,4), device=device)
        ...
        target_poses_9d = {}
        for key, idx, pose in zip(list(range(dof+1)), fk_link_indices, PB_p):
            link = manip_semantic_link_map[key][idx]
        ...
        sim_handle.marionette_robot(joint_config_n_p)

        curr_js = np.copy(joint_config_n_p)
        fk_dict = urdf_handle.link_fk(cfg=joint_config_n_p, use_names=True)  
